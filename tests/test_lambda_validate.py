...
def test_lambda_handler_no_execution_timestamp(mock_lambda_func):
    event = {}
    with pytest.raises(ValueError, match="Execution timestamp must be provided in the event data"):
...
def test_lambda_function_success(mock_predictit_response, mock_s3_client, mock_getenv):
    ...
    mock_s3_client.get_object.assert_called_once_with(Bucket="fake-bucket", Key=f'predictit/stage/market_data_{test_execution_timestamp}.json')
    mock_predictit_response.assert_called_once_with(**test_data)
    mock_s3_client.copy_object.assert_called_once_with(
        Bucket="fake-bucket",
        CopySource={'Bucket': "fake-bucket", 'Key': f"predictit/stage/market_data_{test_execution_timestamp}.json"},
        Key=f"predictit/raw_data/market_data_{test_execution_timestamp}.json"
    )
    mock_s3_client.delete_object.assert_called_once_with(Bucket="fake-bucket", Key=f"predictit/stage/market_data_{test_execution_timestamp}.json")